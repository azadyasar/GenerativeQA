from qa.training import TransformerModelConfig
from qa.data import Dataset
from qa.util import get_lr, epoch_time
...
class Trainer(object):
  ...
  def train(self,
    ...
    for epoch in range(self.n_epochs):
      ...
      if valid_loss < best_valid_loss:
        logger.info(f"Validation loss improved {best_valid_loss:.4f} -> {valid_loss:.4f}. Saving model..")
  ...
  def train_(self, train_dataset: Dataset):
    ...
    epoch_loss = 0
    batch_count = 0
    for batch in tqdm(train_dataset.generate(self.config.batch_sz)):
      batch_count += 1
      ...
      output, _ = self.model(src, trg[:, :-1]) # TODO
